...
For the daily steps, the mean value is 9354.23 steps and the median value is 1.0395\times 10^{4} steps.

As can be seen in the histogram, there is also quite a variation of the daily total step count. The standard deviation is 5405.9.
...
![](PA1_template_files/figure-html/task3-1.png) 

On average, across all the days in the dataset, the interval of 835 contains the maximum number of steps, which is 206.2.
...
In the original data, there were 2304 NA.

All of these NA were replaced with the mean for all interval, which is 37.4 steps.

For comparison purpose, we reprint the original mean and median total steps values. Before the NA were replaced, the mean is 9354.23 steps and the median is the 1.0395\times 10^{4} steps.
After imputting the NA with the mean for all interval, the new mean value is 1.076619\times 10^{4} steps and the new median value is 1.076619\times 10^{4} steps.
...
![](PA1_template_files/figure-html/task5-1.png) 
